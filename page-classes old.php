...
get_header();
$data = array( 'OrgId' => 526868,'Sort'=>'session, category1, category2, category3'); 
//sort by session then by name(cat1) because we want to group the names together
...
if($decoded['rows'] != null){
    for ($i = 0; $i < count($decoded['rows']); $i++) { 
        ...
        //do not include if it is PD Days or camps, so remove the row
        if($decoded['rows'][$i]['category1'] == "PD Days" ||
        $decoded['rows'][$i]['category1'] == "Camps"){
            //remove the row 
            unset($decoded['rows'][$i]);
        }
    }

    //reset array pointer to first element of the array
    reset($decoded['rows']);
    ...
    <div class="container-margin">
        ...
        <?php for($j = 0; $j < count($sessionNames); $j++){
            ...
            ?>
            <h3 class="margin-top-large"><?php echo $sessionNames[$j]; ?></h3>
        ...
        <table>
            ...
            <tbody>
                ...
                <tr>
                        ...
                        <div class="flex flex-col">
                            ...
                            <span>
                                <?php //if(date('Y-m-d') > $array_classes[$i]['regStartDate'] ||
                                    //(date('Y-m-d') == $array_classes[$i]['regStartDate'] && 
                                    //time() >= strtotime("12:00:00"))) {
                                        if ($array_classes[$i]['canRegister']){
                    ...
                    <td class="time">
                        <?php echo $array_classes[$i]['startTime']; ?> - <?php echo $array_classes[$i]['endTime']; ?></td>
